...
class admin_model extends CI_Model {
	...
	function CekAdminLogin($username,$password) {
		...
		if (count($ceklogin->result())>0) {
			...
			foreach ($ceklogin->result() as $value) {
				...
				$sess_data['kode_user']  	= $value->kode_admin;
				$sess_data['nama_user']  	= $value->nama_admin;
				$sess_data['username']  	= $value->username;
				$sess_data['password']  	= $value->password;
				$sess_data['email']  		= $value->email;
				$sess_data['alamat']  		= $value->phone;
				$sess_data['phone']  		= $value->alamat;
	...
	function GetAdmins ($username) {
		...
		return $this->db->query("select * from tbl_admin where username='$username' ");
	}
	...
	function DeleteJam($id_jam) {
		return $this->db->query("delete from tbl_jam where id_jam='$id_jam' ");
	}

	function GetJamEdit($id_jam) {
		return $this->db->query("select * from tbl_jam where id_jam='$id_jam' ");
	}
	//Ahir Jam
	...
	function UpdateCarabooking($id_carabooking,$judul,$deskripsi){
		return $this->db->query("update tbl_carabooking set judul='$judul',deskripsi='$deskripsi' where id_carabooking='$id_carabooking'");
	}
	//Akhir Tentang Kami
	...
	function GetMaxKodeAdmin() {

		$query = $this->db->query("select MAX(RIGHT(kode_admin,3)) as kode_admin from tbl_admin");
		$kode ="";
		if($query->num_rows()>0) {
			foreach ($query->result() as $tampil) {
				$kd = ((int)$tampil->kode_admin)+1;
				$kode = sprintf("%03s",$kd);
			}
		}
		else {
	...
	function Deleteadmin($kode) {
		return $this->db->query("delete from tbl_admin where kode_admin='$kode'");
	}
	...
	//Awal member
	function Getmember() {
		return $this->db->query("select *from tbl_member order by id_member asc");
	}
	...
	function Deletemember($id) {
		return $this->db->query("delete from tbl_member where id_member='$id'");
	}
	...
	function GetLapangan() {
		return $this->db->query("select * from tbl_lapangan order by id_lapangan asc");
	}

	function GetMaxKodeLapangan() {

		$query = $this->db->query("select MAX(RIGHT(kode_lapangan,3)) as kode_lapangan from tbl_lapangan");
		$kode ="";
		if($query->num_rows()>0) {
			foreach ($query->result() as $tampil) {
				$kd = ((int)$tampil->kode_lapangan)+1;
				$kode = sprintf("%03s",$kd);
			}
		}
		else {
	...
	function DeleteLapangan($id_lapangan) {
		return $this->db->query("delete from tbl_lapangan where id_lapangan='$id_lapangan' ");
	}

	function EditLapangan($id_lapangan){
	...
	function GetSlider() {
		return $this->db->query("select * from tbl_slider order by id_slider asc");
	}
	...
	function GetTransaksi(){
		...
		join tbl_bank d on a.bank_id=d.id_bank where a.status='Belum Konfirmasi' order by a.id_transaksi asc");
	}
	
	function UpdateTransaksi($id_transaksi) {
		return $this->db->query("update tbl_transaksi set status='Booking' where id_transaksi='$id_transaksi'  ");
	}
	...
	function GetTransaksiSudah(){
		...
		join tbl_member b on a.member_id=b.id_member
		join tbl_lapangan c on a.lapangan_id=c.id_lapangan 
		join tbl_bank d on a.bank_id=d.id_bank where a.status='Booking' order by a.id_transaksi asc");
	}

	function DetailTransaksiSudah($id_transaksi) {
		return $this->db->query("select a.*,b.*,c.*,d.* from tbl_transaksi a
		join tbl_member b on a.member_id=b.id_member
		join tbl_lapangan c on a.lapangan_id=c.id_lapangan 
		join tbl_bank d on a.bank_id=d.id_bank where id_transaksi='$id_transaksi'");
	...
	function DeleteTransaksiSudah($id_transaksi) {
		return $this->db->query("delete from tbl_transaksi where id_transaksi='$id_transaksi' ");
	}
	...
	function GetAllTransaksi(){
		...
		join tbl_member b on a.member_id=b.id_member
		join tbl_lapangan c on a.lapangan_id=c.id_lapangan 
		join tbl_bank d on a.bank_id=d.id_bank order by a.id_transaksi asc");
	}
	...
	function GetKonfirmasi() {
		return $this->db->query("select a.*,b.* from tbl_konfirmasi a 
		join tbl_bank b on a.bank_id=b.id_bank order by id_konfirmasi asc");
	}
